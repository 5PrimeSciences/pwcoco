...
int run_pwcoco(const PwCoCoConfig &cfg) {
    ...
    if (fs::is_directory(path, ec)) {
        ...
        spdlog::warn("Passing folders as the summary statistics arguments is a beta feature and may be buggy. Please "
                     "do not use this feature to inform on active analyses!");
    ...
    if (data_folder) {
        ...
        for (const auto &dir_entry : recursive_directory_iterator(cfg.sum_stats1)) {
            string filename{dir_entry.path().filename().u8string()}, path_to_file1{dir_entry.path().u8string()};

            for (const auto &dir_entry2 : recursive_directory_iterator(cfg.sum_stats2)) {
                string filename2{dir_entry2.path().filename().u8string()}, path_to_file2{dir_entry2.path().u8string()};

                if (!cfg.pairwise && !path_to_file1.compare(path_to_file2)) {
                    continue;
                }

                unique_ptr<phenotype> exposure(init_pheno(path_to_file1, filename + (cfg.pairwise ? "" : ".exp"),
                                                          cfg.n1, cfg.n1_case, cfg.pve1, cfg.pve_file1));
                unique_ptr<phenotype> outcome(init_pheno(path_to_file2, filename2 + (cfg.pairwise ? "" : ".out"),
                                                         cfg.n2, cfg.n2_case, cfg.pve2, cfg.pve_file2));
                if (exposure->has_failed() || outcome->has_failed()) {
                    spdlog::error(
                        "Reading of either summary statistic files has failed; have these been moved or altered?");
                ...
                if (pwcoco_sub(exposure.get(), outcome.get(), ref.get(), cfg.p_cutoff1, cfg.p_cutoff2, cfg.collinear,
                               cfg.ld_window, cfg.out, cfg.top_snp, cfg.freq_threshold, cfg.cond_ssize, cfg.out_cond,
                               cfg.p1, cfg.p2, cfg.p3, cfg.verbose)) {
                    continue;
                }
            }
        }
    } else {
        unique_ptr<phenotype> exposure(init_pheno(cfg.sum_stats1, fs::path(cfg.sum_stats1).filename().string(), cfg.n1,
                                                  cfg.n1_case, cfg.pve1, cfg.pve_file1));
        unique_ptr<phenotype> outcome(init_pheno(cfg.sum_stats2, fs::path(cfg.sum_stats2).filename().string(), cfg.n2,
                                                 cfg.n2_case, cfg.pve2, cfg.pve_file2));
        if (exposure->has_failed() || outcome->has_failed()) {
        ...
        if (pwcoco_sub(exposure.get(), outcome.get(), ref.get(), cfg.p_cutoff1, cfg.p_cutoff2, cfg.collinear,
                       cfg.ld_window, cfg.out, cfg.top_snp, cfg.freq_threshold, cfg.cond_ssize, cfg.out_cond, cfg.p1,
                       cfg.p2, cfg.p3, cfg.verbose)) {
            return 0;
        }
    }

    chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    spdlog::info("Analysis finished. Computational time: {} secs",
                 (std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) / 1000000.0);
    return 0;
}

int initial_coloc(phenotype *exposure, phenotype *outcome, string out, double p1, double p2, double p3,
                  double init_h4) {
    ...
    if (initial->pp_abf[H4] > init_h4) {
        spdlog::info(
            "Stopping algorthim as H4 for initial colocalisation analysis is already at or above threshold ({}%).",
            init_h4 * 100);
        delete initial;
        return 1;
    }
...
int pwcoco_sub(phenotype *exposure, phenotype *outcome, reference *ref, double p_cutoff1, double p_cutoff2,
               double collinear, double ld_window, string out, double top_snp, double freq_threshold, double cond_ssize,
               bool out_cond, double p1, double p2, double p3, bool verbose) {
    conditional_dat *exp_cdat = new conditional_dat();
    conditional_dat *out_cdat = new conditional_dat();

    cond_analysis *exp_analysis = new cond_analysis(p_cutoff1, collinear, ld_window, out, top_snp, freq_threshold,
                                                    exposure->get_phenoname(), cond_ssize, verbose);
    exp_analysis->init_conditional(exposure, ref);
    exp_analysis->find_independent_snps(exp_cdat, ref);

    cond_analysis *out_analysis = new cond_analysis(p_cutoff2, collinear, ld_window, out, top_snp, freq_threshold,
                                                    outcome->get_phenoname(), cond_ssize, verbose);
    ...
    if (exp_analysis->get_num_ind() == 0 && out_analysis->get_num_ind() == 0) {
        spdlog::warn("Both conditional analyses failed to run or find any conditionally independednt signals, and so "
                     "no colocalisation will be run.");
        delete exp_analysis;
        delete out_analysis;
        delete exp_cdat;
        delete out_cdat;
    ...
    spdlog::info("There are {} selected SNPs in the exposure dataset and {} in the outcome dataset.",
                 exp_analysis->get_num_ind(), out_analysis->get_num_ind());
    spdlog::info("Performing {} conditional and colocalisation analyses.",
                 (exp_analysis->get_num_ind() == 0 ? 1 : exp_analysis->get_num_ind()) *
                     (out_analysis->get_num_ind() == 0 ? 1 : out_analysis->get_num_ind()));
    ...
    for (int i = 0; i < exp_analysis->get_num_ind() + 1; i++) {
        ...
        for (int j = 0; j < out_analysis->get_num_ind() + 1; j++) {
            ...
            conditional_coloc->init_coloc(exp_snp_name, out_snp_name, exp_analysis->get_cond_name(),
