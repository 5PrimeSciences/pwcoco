...
struct conditional_dat {
    ...
    // Initialiser
    conditional_dat()
        : B{0, 0}, B_i{0, 0}, B_N{0, 0}, B_N_i{0, 0}, D_N(0), Z{0, 0},
          Z_N{0, 0} {}; // TODO All will be resized later which may be inefficient
};

class cond_analysis {
  public:
    cond_analysis(double p_cutoff, double collinear, double ld_window, string out, double top_snp, double freq_thres,
                  string name, bool cond_ssize, bool verbose);
    cond_analysis();
  ...
  private:
    ...
    void stepwise_select(vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat, eigenVector &bC,
                         eigenVector &bC_se, eigenVector &pC, reference *ref);

    bool select_entry(vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat, eigenVector &bC,
                      eigenVector &bC_se, eigenVector &pC, reference *ref);
    void selected_stay(vector<size_t> &select, conditional_dat *cdat, eigenVector &bJ, eigenVector &bJ_se,
                       eigenVector &pJ, reference *ref);
    void massoc_conditional(const vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat,
                            eigenVector &bC, eigenVector &bC_se, eigenVector &pC, reference *ref);
    void massoc_joint(const vector<size_t> &idx, conditional_dat *cdat, eigenVector &bJ, eigenVector &bJ_se,
                      eigenVector &pJ, reference *ref);
    ...
    void sanitise_output(vector<size_t> &selected, vector<size_t> &remain, int pos, conditional_dat *cdat,
    ...
    size_t num_ind_snps; // Amount of SNPs selected after stepwise selection; can be 0 if the unconditional data are to
                         // be used or -1 so no SNPs are excluded in the conditional
