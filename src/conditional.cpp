 ...
 */
cond_analysis::cond_analysis(double p_cutoff, double collinear, double ld_window, string out, double top_snp,
                             double freq_thres, string name, bool cond_ssize, bool verbose) {
    cname = name;
    a_out = out;
    a_p_cutoff = p_cutoff;
...
void cond_analysis::match_gwas_phenotype(phenotype *pheno, reference *ref) {
    ...
    for (i = 0; i < pheno_snps.size(); i++) {
        if ((iter = ref->snp_map.find(pheno_snps[i])) == ref->snp_map.end() ||
            (pheno->allele1[i] != ref->bim_allele1[iter->second] &&
             (pheno->allele1[i] != ref->bim_allele2[iter->second]))) {
    ...
    if (unmatched && verbose) {
        spdlog::info("[{}] There were {} SNPs that had a large difference in the allele frequency to that of the "
                     "reference sample.",
                     pheno->get_phenoname(), unmatched);
        ...
        for (i = 0; i < bad_idx.size(); i++) {
            double freq =
                (iter = id_map.find(ref->bim_snp_name[bad_idx[i]])) == id_map.end() ? -1.0 : pheno->freq[iter->second];
            file << ref->bim_snp_name[bad_idx[i]] << "\t" << ref->bim_allele1[bad_idx[i]] << "\t"
                 << ref->bim_allele2[bad_idx[i]] << "\t" << ref->ref_A[bad_idx[i]] << "\t" << mu[bad_idx[i]] / 2.0
                 << "\t" << freq << endl;
        }
        file.close();
    }
    ...
    if (to_include.empty()) {
        spdlog::critical(
            "Included list of SNPs is empty - could not match SNPs from phenotype file with reference SNPs.");
        return;
    } else {
        spdlog::info("[{}] Total amount of SNPs matched from phenotype file with reference SNPs are: {}.", cname,
                     to_include.size());
    }
    ...
    for (size_t j = 0; j < ja_snp_name.size(); j++) {
        file << ja_snp_name[j] << "\t" << ref->bim_bp[to_include[j]] << "\t" << ja_chisq[j] << "\t" << ja_beta[j]
             << "\t" << ja_beta_se[j] << "\t" << ja_pval[j] << "\t" << ja_freq[j] << endl;
    }
    file.close();
    //}
}

void cond_analysis::stepwise_select(vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat,
                                    eigenVector &bC, eigenVector &bC_se, eigenVector &pC, reference *ref) {
    ...
    spdlog::info("[{}] Selected SNP {} with chisq {:.2f} and pval {:.2e}.", cname, ja_snp_name[m], ja_chisq[m],
                 ja_pval[m]);
...
bool cond_analysis::insert_B_Z(const vector<size_t> &idx, size_t pos, conditional_dat *cdat, reference *ref) {
    ...
    size_t i = 0, j = 0, p, n = fam_ids_inc.size(), m = to_include.size();
    double d_temp = 0.0;
        ...
        for (i = j + 1; i < ix.size(); i++) {
            ...
            if (pos == ix[j] || pos == ix[i]) {
                if ((ref->bim_chr[to_include[ix[i]]] == ref->bim_chr[to_include[ix[j]]] &&
                    ...
                    cdat->B_N.insertBack(i, j) = d_temp * min(nD[ix[i]], nD[ix[j]]) *
                                                 sqrt(msx[ix[i]] * msx[ix[j]] / (msx_b[ix[i]] * msx_b[ix[j]]));
                }
            } else {
                size_t ins_row_val = get_ins_row ? 1 : 0, ins_col_val = get_ins_col ? 1 : 0;
    ...
    if (ldlt_B.vectorD().minCoeff() < 0 || sqrt(ldlt_B.vectorD().maxCoeff() / ldlt_B.vectorD().minCoeff()) > 30 ||
        (1 - eigenVector::Constant(ix.size(), 1).array() / (diagB.array() * cdat->B_i.diagonal().array())).maxCoeff() >
            a_collinear) {
    ...
    for (j = 0; j < m; j++) {
        ...
        for (i = 0; i < ix.size(); i++) {
            if (pos == ix[i]) {
                if ((ix[i] != j && ref->bim_chr[to_include[ix[i]]] == ref->bim_chr[to_include[j]] &&
                    ...
                    cdat->Z_N.insertBack(i, j) =
                        d_temp * min(nD[ix[i]], nD[j]) * sqrt(msx[ix[i]] * msx[j] / (msx_b[ix[i]] * msx_b[j]));
                }
                get_ins_row = true;
            } else {
...
void cond_analysis::erase_B_and_Z(const vector<size_t> &idx, size_t erase, conditional_dat *cdat) {
    bool get_ins_col = false, get_ins_row = false;
    size_t i = 0, j = 0, i_size = idx.size(), pos = find(idx.begin(), idx.end(), erase) - idx.begin(),
           m = to_include.size();
...
bool cond_analysis::select_entry(vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat,
                                 eigenVector &bC, eigenVector &bC_se, eigenVector &pC, reference *ref) {
    size_t m = 0;
...
void cond_analysis::selected_stay(vector<size_t> &select, conditional_dat *cdat, eigenVector &bJ, eigenVector &bJ_se,
                                  eigenVector &pJ, reference *ref) {
    if (cdat->B_N.cols() < 1) {
...
void cond_analysis::massoc_conditional(const vector<size_t> &selected, vector<size_t> &remain, conditional_dat *cdat,
                                       eigenVector &bC, eigenVector &bC_se, eigenVector &pC, reference *ref) {
...
void cond_analysis::makex_eigenVector(size_t j, eigenVector &x, bool resize, reference *ref) {
    size_t n = fam_ids_inc.size(), m = to_include.size();
...
bool cond_analysis::init_b(const vector<size_t> &idx, conditional_dat *cdat, reference *ref) {
    size_t i = 0, j = 0, k = 0, n = fam_ids_inc.size(), i_size = idx.size();
    double d_temp = 0.0;
    eigenVector diagB(i_size), x_i(n), x_j(n);
    ...
    for (i = 0; i < i_size; i++) {
        ...
        for (j = i + 1; j < i_size; j++) {
            if ((ref->bim_chr[to_include[idx[i]]] == ref->bim_chr[to_include[idx[j]]] &&
                ...
                cdat->B_N.insertBack(j, i) = d_temp * min(nD[idx[i]], nD[idx[j]]) *
    ...
    cdat->B_i.setIdentity();
    cdat->B_i = ldlt_B.solve(cdat->B_i).eval();
    if ((1 - eigenVector::Constant(i_size, 1).array() / (diagB.array() * cdat->B_i.diagonal().array())).maxCoeff() >
        a_collinear)
        return false;
...
void cond_analysis::init_z(const vector<size_t> &idx, conditional_dat *cdat, reference *ref) {
    size_t i = 0, j = 0, n = fam_ids_inc.size(), m = to_include.size(), i_size = idx.size();
    double d_temp = 0.0;
    eigenVector x_i(n), x_j(n);
    ...
    for (j = 0; j < m; j++) {
        ...
        makex_eigenVector(j, x_j, false, ref);
        for (i = 0; i < i_size; i++) {
            if ((idx[i] != j && ref->bim_chr[to_include[idx[i]]] == ref->bim_chr[to_include[j]] &&
                ...
                cdat->Z.insertBack(i, j) = d_temp;
                cdat->Z_N.insertBack(i, j) =
                    d_temp * min(nD[idx[i]], nD[j]) * sqrt(msx[idx[i]] * msx[j] / (msx_b[idx[i]] * msx_b[j]));
            }
        }
    }
...
void cond_analysis::massoc_joint(const vector<size_t> &idx, conditional_dat *cdat, eigenVector &bJ, eigenVector &bJ_se,
                                 eigenVector &pJ, reference *ref) {
...
void cond_analysis::find_independent_snps(conditional_dat *cdat, reference *ref) {
    ...
    spdlog::info("[{}] Performing stepwise model selection on {} SNPs; p cutoff = {}, collinearity = {} assuming "
                 "complete LE between SNPs more than {} Mb away).",
                 cname, to_include.size(), a_p_cutoff, a_collinear, a_ld_window / 1e6);
    stepwise_select(selected, remain, cdat, bC, bC_se, pC, ref);

    if (selected.empty()) {
        spdlog::warn(
            "[{}] No SNPs have been selected by the step-wise selection algorithm. Using the unconditioned dataset.",
            cname);
...
void cond_analysis::LD_rval(const vector<size_t> &idx, eigenMatrix &rval, conditional_dat *cdat) {
    size_t i = 0, j = 0, i_size = idx.size();
...
void cond_analysis::LD_rval(const vector<size_t> &v1, const vector<size_t> &v2, eigenMatrix &rval, reference *ref) {
    size_t i = 0, j = 0, k = 0, n = fam_ids_inc.size(), v1_size = v1.size(), v2_size = v2.size();
    double d_temp = 0.0;
    eigenVector x_i(n), x_j(n);
...
void cond_analysis::sanitise_output(vector<size_t> &selected, vector<size_t> &remain, int pos, conditional_dat *cdat,
                                    eigenVector &bJ, eigenVector &bJ_se, eigenVector &pJ, reference *ref) {
    string filename;
    size_t i = 0, j = 0, k;
    ...
    for (i = 0; i < remain.size(); i++) {
        j = remain[i];
        ofile << ref->bim_chr[to_include[j]] << "\t" << ref->bim_snp_name[to_include[j]] << "\t"
              << ref->bim_bp[to_include[j]] << "\t";
...
mdata::mdata(cond_analysis *ca1, cond_analysis *ca2) {
    ...
    for (it = ca1->snps_cond.begin(); it != ca1->snps_cond.end(); it++) {
        ...
        if ((it2 = find(ca2->snps_cond.begin(), ca2->snps_cond.end(), *it)) != ca2->snps_cond.end()) {
            size_t dist1 = distance(ca1->snps_cond.begin(), it), dist2 = distance(ca2->snps_cond.begin(), it2);
