...
void phenotype::read_phenofile(string filename) {
    ...
    while (getline(pfile, line)) {
        ...
        if (!delim_found) {
            spdlog::critical(
                "Cannot determine delimiter for phenotype file \"{}\". Use either tab, comma or space-delimited.",
                filename);
            failed = true;
            return;
        }
        ...
        while (getline(ss, substr, sep)) {
            ...
            case 7: // Eighth column contains N total
                ...
                if (n_buf && n_from_cmd && !n_warning) {
                    n_warning = true;
                    spdlog::warn("N is given at command line (n = {}) but the file also contains these values. Using "
                                 "values from command line instead.",
                                 n_from_cmd);
                }
                break;
            case 8: // Nineth column contains N of cases (optional)
                ...
                if (nc_buf && n_case_from_cmd && !n_warning) {
                    n_warning = true;
                    spdlog::warn("N or n_case is given at command line (n_case = {}) but the file also contains these "
                                 "values. Using values from command line instead.",
                                 n_case_from_cmd);
                }
        ...
        if (se_buf == 0.0 || se_buf == 1.0 || freq_buf == -1.0 || beta_buf == 1.0)
            continue;
        ...
        if (s < 0 || s >= 1) {
            spdlog::warn("SNP {} in phenotype file {} has case proportion outside of range [0, 1) - capping.",
                         snp_name_buf, s);
        ...
        if (pheno_variance < 0.0) {
            if (!pve_warning) {
                spdlog::warn("It is preferable to use the full summary statistics to estimate the phenotype variance; "
                             "you can ignore this warning if the summary statistics file you have provided are the "
                             "full summary statistics.");
                spdlog::warn("You can calculate the full phenotype variance by using either the '--pve' flag or "
                             "'--pve_file' flag.");
                pve_warning = true;
            }
            ...
            if (Vp < 0.0) {
                spdlog::critical("Error in reading phenotype file {}: variance is less than zero (Vp = {:.2f}).",
                                 filename, Vp);
                failed = true;
            }
            Vp_v.push_back(Vp);
...
void phenotype::calc_pheno_variance(string pve_file) {
    ...
    while (getline(pfile, line)) {
        ...
        if (!delim_found) {
            spdlog::critical("Cannot determine delimiter for file \"{}\". Use either tab, comma or space-delimited.",
                             pve_file);
            failed = true;
            return;
        }
        ...
        if (Vp < 0.0) {
            spdlog::critical("Error in reading phenotype file {}: variance is less than zero (Vp = {:.2f}).", pve_file,
                             Vp);
            failed = true;
        }
        Vp_v.push_back(Vp);
...
int reference::read_bimfile(string bimfile) {
    ...
    while (bim.good()) {
        ...
        transform(bim_allele1_buf.begin(), bim_allele1_buf.end(), bim_allele1_buf.begin(),
                  ::toupper); // @TODO Is it quicker to just apply this to the entire vector after?
...
void reference::match_bim(vector<string> &names, vector<string> &names2, bool keep_frequencies) {
    // Temporary containers
    vector<string> bim_snp_name_t, bim_allele1_t, bim_allele2_t;
...
void reference::pair_fam() {
    ...
    for (i = 0; i < individuals; i++) {
        ...
        if (size == fam_map.size())
            spdlog::warn("Duplicate individual in .fam file found: {}, {}.", fam_fid[i],
                         fam_iid[i]); /// Include IDs here probably
        size = fam_map.size();
    }
}
...
int reference::filter_snp_maf(double maf) {
    ...
    // stable_sort(to_include.begin(), to_include.end());
    spdlog::info("After filtering based on MAF, there are {} SNPs remaining in the analysis (amount removed: {}).",
                 to_include.size(), prev_size - to_include.size());
...
void reference::parse_bed_data(char *buf, size_t snp_idx, vector<int> read_individuals) {
    size_t j, k, m = fam_ids_inc.size(), ind_idx;
    int buf_count = 0;
    bitset<8> b;
    double fcount = 0;
...
int reference::read_bedfile(string bedfile) {
    size_t i;
    const size_t bim_size = to_include.size(), fam_size = fam_ids_inc.size();
