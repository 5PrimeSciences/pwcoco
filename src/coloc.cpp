...
bool coloc_analysis::estimate_bf(const vector<double> beta, const vector<double> se, const vector<double> freq,
    ...
    if (type == coloc_type::COLOC_QUANT) {
        ...
        // Estimate sdY from
        transform(nvx.begin(), nvx.end(), ssize.begin(), nvx.begin(),
                  [](double x, double n_) { return 2 * n_ * x * (1 - x); });

        // Regress n*var(x) against 1/var(beta)
        sdY = lm_fixed(invbeta, nvx); // same as: lm(nvx ~ invbeta - 1)
        if (sdY < 0) {
            spdlog::critical("sdY estimation is negative (sdY = {:.2f}) which may be caused by small datasets or those "
                             "with errors. Cannot continue with colocalisation analysis.",
                             sdY);
            return false;
        }
        sdY = sqrt(sdY);
    ...
    transform(r.begin(), r.end(), r.begin(),
              [sd_prior](double v) { return (sd_prior * sd_prior) / (sd_prior * sd_prior + v); });
...
void coloc_analysis::combine_abf(size_t abf_size) {
    double log_ABF1 = logsum(*ABF_1), log_ABF2 = logsum(*ABF_2);
...
void coloc_analysis::init_coloc(string exp, string out) {
    if (matched->snps1.size() <= 0 || matched->snps2.size() <= 0) {
        spdlog::warn(
            "Could not conduct colocalisation analysis for {} and {} as no SNPs were included in the analysis.", exp,
            out);
        return;
    }
    ...
    spdlog::info("H0: {:.2f}; H1: {:.2f}; H2: {:.2f}; H3: {:.2f}; H4: {:.2f}; abf_all: {:.2f}.", pp_abf[H0], pp_abf[H1],
                 pp_abf[H2], pp_abf[H3], pp_abf[H4], log_abf_all);
...
void coloc_analysis::results_to_file(string s1, string s2, string exp, string out) {
    ...
    if (file.fail()) {
        spdlog::warn("Could not write colocalisation results to file {}. Please check permissions for this folder.",
                     outfile + ".coloc");
        return;
    }
    ...
    file << exp << "\t" << out << "\t" << s1 << "\t" << s2 << "\t" << num_snps() << "\t" << pp_abf[H0] << "\t"
         << pp_abf[H1] << "\t" << pp_abf[H2] << "\t" << pp_abf[H3] << "\t" << pp_abf[H4] << "\t" << log_abf_all << endl;
    file.close();
}
